...
void setup() {
  ...
  // Sync RTC if needed
  if (conf.AUTO_SYNC_RTC && rtc.lostPower() && WiFi.status() == WL_CONNECTED) {
...
void loop() {
  ...
  if (now - lastUpdateMillis >= 1000) {
    Serial.println("1 sec triggered");
    ...
    if (!isBoot && !rtc.lostPower()) {
      ...
      if (now - lastPublish >= (conf.REPORT_INTERVAL * 1000)) {
        lastPublish = now;
        Serial.println("Report interval");
        if (readPzem() && client.connected()) {
          Serial.println("Read PZEM data");
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (!error && doc.containsKey("cmd")) {
    ...
    if (cmd == "pzem-reset") {
      ...
      syncRTC();
      client.publish(conf.MQTT_OUT_STATUS, "{\"status\":\"RTC synced\"}");
    ...
    case 1:
      {
        ...
        // display.fillRect(0, map(screenIndex, 0, menuTotal - 1, 12, 13 + (7 * (menuTotal - 1))), 4, 4, WHITE);
          ...
          case 0:
            {
              ...
              // 2. Hitung tiap satuan menggunakan modulo (%)
