...
class DataRealtime(Base):
    ...
    device_id = Column(BigInteger, ForeignKey("devices.id"), primary_key=True) # It seems 1-to-1 often for realtime status of device? schema image shows data_realtime has device_id. The repo upsert logic suggests one row per device.
